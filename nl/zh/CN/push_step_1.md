...
3. 在导航窗格中，选择**设置** > **项目设置**。
4. 选择“云消息传递”选项卡，以获取项目凭证 -“服务器 API 密钥”和“发送方标识”。请注意，FCM 中列出的“服务器密钥”与“服务器 API 密钥”相同。
   
	![获取 FCM 的凭证](images/FCM_settings_2.jpg)
...
3. 在 **Registering App IDs** 页面中 App ID Description 下的 Name 字段中，提供应用程序名称。例如：ACME Push Notifications。
4. 为 App ID Prefix 提供字符串。  
4. 对于 App ID Suffix，选中 **Explicit App ID**，并提供 Bundle ID 值。建议提供逆向域名样式的字符串。例如：`com.ACME.push`。
5. 选中 **Push Notifications** 复选框，然后单击 **Continue**。
6. 完成设置，然后单击 **Register** > **Done**。

现在，您的应用程序标识已注册。 

   ![注册的应用程序标识](images/push_ios_register_appid.jpg)
...
8. 在 Download, Install and Backup 窗格中，单击 **Download**。这将下载 `aps_development.cer` 文件。
	...
	![下载证书](images/push_certificate_download.jpg)	
	
6. 从菜单选择**钥匙串访问 > 证书助理 > 从证书颁发机构请求证书…** 
7. 在**证书信息**中，输入与您的 App Developer 帐户相关联的电子邮件地址和常用名称。提供有意义的名称来帮助您识别此为开发（沙箱）证书还是分发（生产）证书；例如，_sandbox_apns_certificate_ 或 _production_apns_certificate_。
...
18. 在**输入密码**字段中，输入用于保护导出项的密码，然后单击**确定**。您可以使用此密码，在 Push Notifications 服务控制台上配置 APNs 设置。
...
获取凭证后，下一步是[配置服务实例](push_step_2.html)。
...
要为 Chrome、Firefox 浏览器和 Chrome Apps & Extensions 设置该服务，请参阅[配置服务实例](push_step_2.html)。
...
3. 通过选择加号图标来选择创建新条目。
![Push Notifications 控制台](images/safari_1.jpg)
...
生成证书后，下一步是[配置服务实例](push_step_2.html)。
